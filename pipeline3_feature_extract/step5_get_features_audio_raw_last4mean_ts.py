...
MODEL_NAME = "wav2vec2_fairseq_large_lv60k_asr_ls960_last4mean_native_ts"
CKPT_NAME = "wav2vec2_fairseq_large_lv60k_asr_ls960.pth"
VIDEO_EXTS = [".mp4", ".avi"]

TORCH_HOME = Path(PATH.PIPELINE_3_ROOT) / "torch_cache"
WAV_DUMP_ROOT = Path(PATH.OFFAUDIO_WAV2VEC2_PT2)
OUT_ROOT = Path(PATH.OFFAUDIO_WAV2VEC2_PT)

# Chunking: attention ~ O(T^2)
CHUNK_SEC = 20.0
MIN_TAIL_SEC = 1.0
...
def read_expr_ids(csv_path: Path) -> List[str]:
    if not csv_path.exists():
        raise FileNotFoundError(f"Missing csv: {csv_path}")
    with csv_path.open("r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        fieldnames = reader.fieldnames or []
        cand_cols = ["folderid", "video_id", "vid", "id", "name", "video", "videoid", "txtid"]
...
def load_wav2vec2_model_from_local_ckpt(device: torch.device):
    ...
    if len(missing) > 0:
        print("[INFO] first missing keys:", missing[:10], flush=True)
    if len(unexpected) > 0:
        print("[INFO] first unexpected keys:", unexpected[:10], flush=True)

    model = model.to(device).eval()
    return model


# --------------------------- core extraction (chunked & stable) ---------------------------
...
def extract_last4mean_with_timestamps_chunked(
    ...
    """
    Extract last 4 transformer layers, mean them -> [t', 1024] per chunk,
    and generate per-step timestamps (seconds) aligned to waveform chunk.
    ...
    if len(starts) >= 2:
        ...
        if last_len < min_tail:
            starts.pop(-1)  # merge tail into previous chunk
    ...
    for i, s in enumerate(starts):
        ...
        with torch.amp.autocast(device_type="cuda", enabled=use_amp):
            ...
            feat_mean = torch.stack(layers, dim=0).mean(dim=0)  # [t', 1024]
...
@torch.inference_mode()
def main():
    print("[BOOT] step5_get_features_audio_raw_last4mean_ts.py starting.", flush=True)
    ...
    ids = read_expr_ids(Path(PATH.EXPR_VIDEO_INDEX_CSV))
    print(f"[INFO] Loaded EXPR ids: {len(ids)} from {PATH.EXPR_VIDEO_INDEX_CSV}", flush=True)

    miss = 0
    for folderid in tqdm(ids, desc="Total [AUDIO wav2vec2 last4mean nativeTS]"):
