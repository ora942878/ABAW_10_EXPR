...
WIN_SEC = 0.50
FALLBACK_EMPTY = "nearest"


RAW_AUDIO_ROOT = Path(PATH.OFFAUDIO_WAV2VEC2_PT_TEST)
OUT_ROOT = PATH.OFFAUDIO_WAV2VEC2_PT_aligned_window50_TEST
META_CSV = PATH.EXPR_VIDEO_META_CSV_TEST
# =========================== helpers ===========================
def out_checkpoint_is_valid(pt_path: Path, expect_dim: int = 1024) -> bool:
    ...
    try:
        ...
        feats = obj["feats"]
        frames = obj["frames"]
        if (not torch.is_tensor(feats)) or feats.ndim != 2 or int(feats.shape[1]) != expect_dim:
            return False
...
def main():
    ...
    for row in tqdm(df.itertuples(index=False), total=len(df), desc="Align TEST audio -> video frames"):
    ...
    if skips:
        out_skip = Path(PATH.METADATA_ROOT_TEST) / "expr_audio_align_skips_test.csv"
