...
OUTPUT_TXT = PATH.METADATA_ROOT_TEST / "predictions_median101.txt"
...
def nearest_true_index(valid_mask, center_idx):
    true_idx = np.flatnonzero(valid_mask)
    if len(true_idx) == 0:
        return None
    return true_idx[np.argmin(np.abs(true_idx - center_idx))]
...
def load_single_model_and_stats(cfg):
    model = build_model(cfg).to(DEVICE)
    ckpt = torch.load(SINGLE_RUN_DIR / "best.pt", map_location="cpu", weights_only=False)
    model.load_state_dict(ckpt["model"], strict=True)

    stats = [
        torch.from_numpy(np.load(SINGLE_RUN_DIR / "train_vis_mean.npy")).float().to(DEVICE),
        torch.from_numpy(np.load(SINGLE_RUN_DIR / "train_vis_std.npy")).float().to(DEVICE),
        torch.from_numpy(np.load(SINGLE_RUN_DIR / "train_aud_mean.npy")).float().to(DEVICE),
        torch.from_numpy(np.load(SINGLE_RUN_DIR / "train_aud_std.npy")).float().to(DEVICE),
    ]
    stats[1] = torch.clamp(stats[1], min=1e-6)
    stats[3] = torch.clamp(stats[3], min=1e-6)
...
def load_cv5_models_and_stats(cfg):
    ...
    for fold_id in range(1, 6):
        ...
        stats = [
            torch.from_numpy(np.load(fold_dir / "train_vis_mean.npy")).float().to(DEVICE),
            torch.from_numpy(np.load(fold_dir / "train_vis_std.npy")).float().to(DEVICE),
            torch.from_numpy(np.load(fold_dir / "train_aud_mean.npy")).float().to(DEVICE),
            torch.from_numpy(np.load(fold_dir / "train_aud_std.npy")).float().to(DEVICE),
        ]
        stats[1] = torch.clamp(stats[1], min=1e-6)
        stats[3] = torch.clamp(stats[3], min=1e-6)

        bundle.append((model, stats))

    if len(bundle) == 0:
        raise RuntimeError("no fold checkpoint found in CV5_RUN_DIR")

    return bundle


def predict_logits_for_video_single(cfg, rows_this_txtid, vis_maps, aud_map, model, stats):
    v_mean, v_std, a_mean, a_std = stats
    ...
    Vt, At = normalize_features_np(
        V, A, v_mean, v_std, a_mean, a_std,
        do_l2=getattr(cfg, "DO_BRANCH_L2_NORM", False)
    )
    logits = forward_in_batches(model, Vt, At, batch_size=BATCH_SIZE)
    return logits, raw_valid_mask, usable_mask


def predict_logits_for_video_cv5(cfg, rows_this_txtid, vis_maps, aud_map, model_bundle):
    logits_sum = None
    raw_valid_mask_ref = None
    usable_mask_ref = None

    for model, stats in model_bundle:
        logits, raw_valid_mask, usable_mask = predict_logits_for_video_single(
            cfg=cfg,
            rows_this_txtid=rows_this_txtid,
            vis_maps=vis_maps,
            aud_map=aud_map,
        ...
        if logits_sum is None:
            logits_sum = logits
            raw_valid_mask_ref = raw_valid_mask
            usable_mask_ref = usable_mask
        else:
            logits_sum = logits_sum + logits

    if logits_sum is None:
        return None, raw_valid_mask_ref, usable_mask_ref

    logits_mean = logits_sum / float(len(model_bundle))
    return logits_mean, raw_valid_mask_ref, usable_mask_ref
...
def main():
    ...
    print(f"[INFO] USE_CV5_ENSEMBLE = {USE_CV5_ENSEMBLE}")
    ...
    if USE_CV5_ENSEMBLE:
        ...
        print(f"[INFO] loaded {len(model_bundle)} fold models")
    else:
        model, stats = load_single_model_and_stats(cfg)
        print("[INFO] loaded single model")
    ...
    pbar = tqdm(rows_by_txtid.items(), desc="Test inference by txtid")
